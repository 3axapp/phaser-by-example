...
export class FoeGenerator
{
    ...
    public generate()
    {
        if (this.scene.number === 4) {
            ...
            this.generateEvent1 = this.scene.time.addEvent({
                delay   : 7000,
                callback: () => this.orderedWave(), callbackScope: this,
                loop    : true,
            });
            ...
            if (this.scene.number > 1) {
                this.generateEvent3 = this.scene.time.addEvent({
                    delay   : 3000,
                    callback: () => this.tank(), callbackScope: this,
                    loop    : true,
                });
            }
    ...
    private releaseGuinxu()
    {
        ...
        this.scene.tweens.add({
            targets: guinxu,
            alpha  : {from: 0.3, to: 1}, duration: 200,
            repeat : 10,
        });
    ...
    private stop()
    {
        ...
        this.scene.foeGroup.children.entries.forEach(((foe: Foe) =>
        {
            if (foe === null || !foe.active) {
                return;
            }
            foe.destroy();
        }) as any);
    ...
    private orderedWave(difficulty = 5)
    {
    ...
    private slider()
    {
        ...
        const foe = new Foe(
            this.scene,
            x,
            Phaser.Math.Between(100, 600), "foe1",
            velocity,
            0
        );
        this.scene.tweens.add({
    ...
    private add()
    {
        const foe = new Foe(
            this.scene,
            Phaser.Math.Between(32, this.scene.width - 32), 0
        );
        this.scene.foeGroup.add(foe);
    }

    private addOrder(i: number, x: number, y: number, minus: number)
    {
    ...
    public update()
    {
        if (this.path) {
            ...
            this.scene.foeWaveGroup.children.entries.forEach(((foe: Foe) =>
            {
                if (foe === null || !foe.active) {
                    return;
                }
