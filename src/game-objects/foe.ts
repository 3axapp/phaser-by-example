...
type TTypeKey = 'foe0' | 'foe1' | 'foe2' | 'guinxu';
const TYPES: Record<TTypeKey, TType> = {
...
export class Foe extends Phaser.GameObjects.Sprite
{
    ...
    public constructor(scene: Game, x: number, y: number, public name: TTypeKey = "foe0", velocityX = 0, velocityY = 0)
    {
    ...
    private spawnShadow(x: number, y: number)
    {
        this.shadow = this.scene.add
                          .image(x + 20, y + 20, this.name).setScale(0.7)
    ...
    private init()
    {
        this.scene.anims.create({
            key   : this.name,
            frames: this.scene.anims.generateFrameNumbers(this.name), frameRate: 10,
            repeat: -1,
        });
    ...
    public update()
    {
        ...
        if (this.name === "guinxu" && Phaser.Math.Between(1, 6) > 5) {
            ...
            this.scene.physics.moveTo(shot,
                this.scene.player.x, this.scene.player.y, 300
            );
            this.scene.physics.moveTo(
                shot.shadow, this.scene.player.x, this.scene.player.y, 300
            );
        }
        if (this.name !== "foe2") {
            this.updateShadow();
    ...
    private guinxuShot()
    {
        ...
        let shot = new FoeShot(
            ...
            this.y,
            "foe",
            this.name, this.pattern[this.patternIndex],
            300
        );
        this.scene.foeShots.add(shot);
        this.patternIndex =
    ...
    public dead()
    {
        ...
        this.scene.tweens.add({
            ...
            alpha     : {from: 1, to: 0.3}, duration: 250,
            onComplete: () =>
            {
                explosion.destroy();
            },
        ...
        if (
            this.name !== "foe2" && this.scene && this.scene.scene.isActive() && this.shadow && this.shadow.active
        ) {
            this.shadow.destroy();
        }
        if (this.name === "guinxu") {
    ...
    private showPoints(score: number, color = 0xff0000)
    {
        ...
        this.scene.tweens.add({
            ...
            duration: 800,
            alpha   : {from: 1, to: 0},
            y       : {from: this.y - 20, to: this.y - 80}, onComplete: () =>
            {
                text.destroy();
            },
        });
