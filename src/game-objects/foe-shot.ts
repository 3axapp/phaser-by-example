...
type TTypeKeys = keyof typeof TYPES;

export class FoeShot extends Phaser.GameObjects.PointLight
{
    ...
    spawnShadow(x: number, y: number, velocityX: number, velocityY: number)
    {
        ...
        if (this.playerName === "guinxu") {
            (this.shadow.body as any).setVelocity(velocityX, velocityY);
        }
    }
