import {Scene} from "phaser";
import {Player} from "../game-objects/player";
import {Generator} from "../game-objects/generator";
import {Obstacle} from "../game-objects/obstacle";
import {Coin} from "../game-objects/coin";

export class Game extends Scene
{

    public height: number;
    public obstacles: Phaser.GameObjects.Group;
    public coins: Phaser.GameObjects.Group;

    private name: string;
    private number: number;
    private player: Player;
    private score = 0;
    private theme: any;
    private jumpTween: Phaser.Tweens.Tween;
    private width: number;
    private center_width: number;
    private center_height: number;
    private generator: Generator;
    private SPACE: Phaser.Input.Keyboard.Key;
    private scoreText: Phaser.GameObjects.BitmapText;
    private updateScoreEvent: Phaser.Time.TimerEvent;
    private audios: Record<string, Phaser.Sound.NoAudioSound | Phaser.Sound.HTML5AudioSound | Phaser.Sound.WebAudioSound>;

    public constructor()
    {
        super({key: "game"});
    }

    public init(data: { name: string, number: number })
    {
        this.name   = data.name;
        this.number = data.number;
    }

    public preload()
    {
        this.registry.set("score", "0");
        this.load.audio("coin", "assets/sounds/coin.mp3");
        this.load.audio("jump", "assets/sounds/jump.mp3");
        this.load.audio("dead", "assets/sounds/dead.mp3");
        this.load.audio("theme", "assets/sounds/theme.mp3");
        this.load.spritesheet("coin", "./assets/images/coin.png", {
            frameWidth : 32,
            frameHeight: 32,
        });
        this.load.bitmapFont("arcade", "assets/fonts/arcade.png", "assets/fonts/arcade.xml");
        this.score = 0;
    }

    public create()
    {
        this.width         = this.sys.game.config.width as number;
        this.height        = this.sys.game.config.height as number;
        this.center_width  = this.width / 2;
        this.center_height = this.height / 2;
        this.cameras.main.setBackgroundColor(0x87ceeb);
        this.obstacles = this.add.group();
        this.coins     = this.add.group();
        this.generator = new Generator(this);
        this.SPACE     = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.SPACE);
        this.player    = new Player(this, this.center_width - 100, this.height - 200);
        this.scoreText = this.add.bitmapText(
            this.center_width,
            10,
            "arcade",
            String(this.score),
            20
        );

        this.physics.add.collider(
            this.player,
            this.obstacles,
            this.hitObstacle as any,
            () =>
            {
                return true;
            },
            this
        );

        this.physics.add.overlap(
            this.player,
            this.coins,
            this.hitCoin as any,
            () =>
            {
                return true;
            },
            this
        );
        this.loadAudios();
        this.playMusic();

        this.input.on("pointerdown", () => this.jump(), this);
        this.updateScoreEvent = this.time.addEvent({
            delay   : 100,
            callback: () => this.updateScore(), callbackScope: this,
            loop    : true,
        });
    }

    public update()
    {
        if (Phaser.Input.Keyboard.JustDown(this.SPACE)) {
            this.jump();
        } else if ((this.player.body as Phaser.Physics.Arcade.Body).blocked.down) {
            this.jumpTween?.stop();
            this.player.rotation = 0; // ground
        }
    }

    private hitObstacle(player: Player, obstacle: Obstacle)
    {
        this.updateScoreEvent.destroy();
        this.finishScene();
    }

    private hitCoin(player: Player, coin: Coin)
    {
        this.playAudio("coin");
        this.updateScore(1000);
        coin.destroy();
    }

    private loadAudios()
    {
        this.audios = {
            jump: this.sound.add("jump"),
            coin: this.sound.add("coin"),
            dead: this.sound.add("dead"),
        };
    }

    private playAudio(key: string)
    {
        this.audios[key].play();
    }

    private playMusic(theme = "theme")
    {
        this.theme = this.sound.add(theme);
        this.theme.stop();
        this.theme.play({
            mute  : false,
            volume: 1,
            rate  : 1,
            detune: 0,
            seek  : 0,
            loop  : true,
            delay : 0,
        });
    }

    private jump()
    {
        if (!(this.player.body as Phaser.Physics.Arcade.Body).blocked.down) {
            return;
        }
        (this.player.body as Phaser.Physics.Arcade.Body).setVelocityY(-300);
        this.playAudio("jump");
        this.jumpTween = this.tweens.add({
            targets: this.player, duration: 1000,
            angle  : {from: 0, to: 360}, repeat: -1,
        });
    }

    private finishScene()
    {
        this.theme.stop();
        this.playAudio("dead");
        this.registry.set("score", "" + this.score);
        this.scene.start("gameover");
    }

    private updateScore(points = 1)
    {
        this.score += points;
        this.scoreText.setText(String(this.score));
    }
}
